...
Both paths are overridable in config.

A package can also be a **command** — a prompt the user invokes, rather than context the model
selects — by declaring `kind: command` in `skill.yaml`. Commands install as a single
`<name>.md` file into `~/.claude/commands/` or `<project>/.claude/commands/`.

Or a **workflow** (`kind: workflow`): a deterministic script Claude Code runs by name, built
from `phase()`, `agent()` and `parallel()`. It installs as `<name>.js` into
`~/.claude/workflows/` or `<project>/.claude/workflows/`.

Codex has neither, so installing one there is reported as skipped rather than written into a
directory it never reads. Agents are detected automatically:
...
| `create <name>`              | Scaffold a skill, `--kind command`, or `--kind workflow` |
