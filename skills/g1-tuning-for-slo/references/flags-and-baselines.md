...
| ------------------------------------- | --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `-XX:+UseG1GC`                        | ergonomic default since JDK 9 (JEP 248)       | Selects G1                                                                                                                                                                                 | Explicit selection makes intent independent of ergonomics. JDK 27 EA documentation says G1 is the default; JEP 523 is Closed/Delivered for release 27; verify the deployed vendor/build rather than inferring older-runtime behavior — `jvm-gc-tuning` |
...
| `-XX:G1MixedGCLiveThresholdPercent`   | 85, **experimental**                          | Old region with more live data than this is never a candidate                                                                                                                              | Raising it collects denser regions (more copying per byte reclaimed); lowering it can defer reclamation until later liveness cycles or full GC                                                                                                         |
...
The JDK 27 early-access `java` documentation names **`-XX:G1IHOP`** and no longer lists
`-XX:InitiatingHeapOccupancyPercent`. On 25.0.3 `-XX:G1IHOP` is `Unrecognized VM option`
(executed). Do not infer alias or warning behavior from documentation alone: exercise both
...
which avoids G1's concurrent marking/SATB but still tracks cross-generation references. This
