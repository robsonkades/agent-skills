...
| Is this fix worth doing?                     | Amdahl's Law — Amdahl 1967                                                          | A fixed-work decomposition with comparable clocks and unchanged remainder; otherwise a conditional estimate                             |
...
A **hypothesis** predicts an observation: "per-request pattern compilation consumes CPU;
reusing the immutable pattern should reduce that work, and may reduce queueing at this load."
A **measurement** records its method: "CPU samples over 120 s, 800 rps, compilation subtree
at 31% of included samples, n = 6,100". Neither a CPU sample fraction nor a wall-clock profile
across all threads is automatically request-critical-path time. Standard JFR `profile.jfc`
is not a general all-thread wall-clock sampling mode; verify enabled events and profiler mode.

A predicted p99 improvement requires a separate workload/queueing or per-request model and
...
wait, but accumulated contention can produce that wait. Measure queueing and arrival patterns
before accepting or rejecting the lock hypothesis from hold duration alone.
...
- **`p` from the wrong clock.** A CPU profile gives the fraction of _CPU samples_; a latency
  ...
  fraction of a frame is not its share of p99. Use request-correlated critical-path evidence;
  a wall-clock profile aggregated across idle and active threads is not sufficient.
...
Scalability Law can fit contention/coherency-shaped costs, but a fitted coefficient does not
identify the physical cause of "adding pods made it worse"
...
- **One factor at a time, unless factors interact.** Changing one variable per run simplifies
  attribution under controlled conditions, but does not eliminate confounding. It cannot see an interaction — heap size and
...
- **Averaging over the interval.** A five-minute utilisation of 70% is consistent with three
  ...
  same window may also hide ordering; use time-resolved utilization and queue evidence
  matched to the incident to distinguish burst patterns.
...
| ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **The deploy confound**         | The fix shipped with a restart (JIT profile reset, caches emptied, connections re-established, pods rescheduled onto other nodes, heap compacted) and the improvement belongs to one of those | Enumerate deploy side effects; choose a safe counterfactual (existing toggle, restarted control or justified alternative); report remaining confounds           |
| **Regression to the mean**      | Selecting an unusually bad observation makes a later ordinary value look like a treatment benefit                                                                                             | Use comparable periods or contemporaneous controls; cover the relevant workload and pattern; longer duration alone does not establish comparability             |
...
| **Rerunning until it improves** | Runs that showed no gain were discarded as "noisy"; the reported run is the best of `k`                                                                                                       | Predeclare replication/stopping and allocation; report all planned outcomes, using justified randomization/blocking                                             |
...
| **Stopping at the first cause** | A real finding that explains 5% of the gap ends the investigation                                                                                                                             | Use fixed-work bounds only where valid; quantify contributors and interactions without treating endpoint quantiles as additive                                  |
...
| **The wrong environment**       | Staging reproduced the symptom's shape but not its cause, because the data, the hot keys, the concurrency or the uptime differ                                                                | State environment differences and test the discriminating hypothesis; production observation still needs an impact budget                                       |
| **Unfalsifiable hypothesis**    | "It needs more resources", "the code is inefficient" — nothing observable would refute it                                                                                                     | Name the component, mechanism and an observation that could refute it; quantify predicted effects when supported, otherwise state what must be measured         |
...
## Investigation symptom → candidate explanation → next check

| Symptom                                 | What went wrong                                           | Fix                                                                                        |
| --------------------------------------- | --------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| Days in, no hypothesis has been refuted | Weak hypotheses, rare events or collection gaps           | Name the discriminating observation and whether it is obtainable; do not invent a result   |
| Every finding is a contributor          | Several causes or insufficient attribution                | Rank plausible payoff with uncertainty; investigate combinations and interactions          |
| Fix worked in staging only              | Environment, traffic or rollout differs                   | Compare the relevant axes before attributing the failure                                   |
| Improvement vanished                    | Restart, drift, workload change or treatment interaction  | Use a safe counterfactual and matched periods                                              |
| Investigators disagree                  | Different methods, populations or interpretations         | Define a common discriminating measurement with adequate replication                       |
| Profile shows nothing                   | Wrong clock, missing events, loss or insufficient samples | Report coverage and sampling limits; absence of samples is not absence of cost             |
| Result moves with run length            | Warm-up, drift, changing load or accumulated state        | Report time evolution and match production age; do not discard the phenomenon              |
| Team is tuning flags                    | Could be mechanism-led tuning or random change            | Require a flag-specific hypothesis and measured contrast                                   |
| Report lacks uncertainty                | Analysis or replication may be insufficient               | Choose experimental units and precision requirements; three runs is not a universal remedy |
| SLO remains missed                      | Bound assumptions or other mechanisms may matter          | Revisit request cohorts, critical paths, queueing and interactions                         |
...
with those differences and their consequences stated. Choose open arrivals or closed
population load to match the production question; an independent arrival schedule still
requires checking dropped/late starts and client capacity against a production-shaped dataset (`load-testing`).
...
- **Stop pursuing a bounded option when its credible benefit cannot close the gap.** This
  does not prove all local options fail. Consider combinations and interactions; only sum
  disjoint savings on the same fixed-work decomposition with unchanged remainder. Tail
  quantiles, changing critical paths and queueing feedback do not have that simple ceiling.
...
Observations that justify comparing architectural options with local changes; none alone
proves an architectural fix is necessary:
...
- Throughput **falls** as instances or threads are added. USL can describe such a curve;
  shared state, quotas, placement, load-balancer effects or local code may explain it.
  Verify the mechanism rather than diagnosing from the fitted coefficient (`universal-scalability-law`).
