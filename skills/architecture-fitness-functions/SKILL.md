...
**If two competent people read the result and disagree, it is not a fitness function**; every "cannot be
governed automatically" conclusion below is that line applied. The books add only
...
| **C** | high on the number, contested on attribution         | an on-call rota; a false alarm wakes someone              | seconds to minutes | deploys freeze on a spent error budget                    | the largest tests you own, and flakiness rises with test size. Reverse once overrides outnumber fixes                                                  |
| **M** | depends on the written criterion, never on the human | a calendar slot, and a person senior enough to be scarce  | one cadence period | the release does not proceed without the recorded verdict | scarce attention, drift invisible between dates, a criterion loose enough that the verdict is the reviewer's mood. Reverse if only timing blocked it   |
...
| **Architecture test** | structural assertions over a model of the code                             | A subset — always atomic, static, triggered. Having them is not having a suite: the holistic/continual half is missing. `architecture-testing`             |
...
| **SLA**               | a contract with users, carrying consequences                               | Never one; it motivates one. Set your threshold at the contractual level and you have kept no headroom                                                     |
