...
| Method at tier 1 and never higher                                   | Configured C1-only mode, trivial method, or `COMPILE SKIPPED` bailout                                              | Inspect mode/flags and matching failure/task history                                          |
...
| `made not entrant: uncommon trap` repeats on one method             | Repeated speculation failure; determine whether it forms a costly recompilation loop                               | `jdk.Deoptimization` `reason`/`action`, successor tasks and workload changes                  |
| `made not entrant: marked for deoptimization` in a burst            | Class/dependency changes or redefinition; a directive addition alone does not rewrite installed code on this build | Correlate class loading, redefinition/instrumentation and task history                        |
| Tree says `callee is too large` for a 60-byte hot callee            | A C1 size decision; not a C2 verdict                                                                               | Inspect the actual enabled compiler; find a C2 tree only for a C2 question                    |
...
| `already compiled into a big method` after a JDK or code change     | Callee's adjusted compiled instruction size exceeds `InlineSmallCode`                                              | Confirm the target build's metric and compilation; codelist address spans are not that metric |
...
| `CompileCommand=log,C::m` produced no file                          | Needs `-XX:+LogCompilation` too                                                                                    | Add it, or capture inlining with `-Xlog:jit+inlining=debug:file=jit-session.log`              |
