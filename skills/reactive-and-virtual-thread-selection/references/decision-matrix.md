...
| Blocking I/O                   | the intended usage — unmounts, carrier freed               | forbidden; one blocking call stalls an event loop                  |
...
| Memory per in-flight request   | a continuation stack on the heap (KBs, workload-dependent) | subscription + operator state (typically smaller)                  |
| Memory per **idle** connection | a parked stack, if a thread is dedicated to it             | subscription state only — the model's strongest quantitative win   |
...
The often-quoted "virtual threads cost a few hundred bytes" is the _initial_ cost. What a
parked virtual thread actually holds is its frozen stack:
...
At 10 000 concurrent in-flight requests that is single-digit to low hundreds of megabytes —
usually irrelevant next to the request data itself. At 1 000 000 mostly-idle connections it
is the machine size, and that is the regime where the reactive model's smaller per-connection
state is a genuine engineering argument rather than a talking point.
