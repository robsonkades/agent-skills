...
`.metrics()` registers a fixed set of names. They are not derivable by analogy from the
operator that produced them. `%s` is the name given via `.name(...)`, defaulting to
`reactor`.
...
| `%s.flow.duration`    | Timer, tagged `status` (`completed`/`error`/`cancelled`) | Duration of the whole sequence, from `subscribe()` to termination                                                                                             |
...
`ReactorDebugAgent` (`io.projectreactor:reactor-tools`) delivers the same assembly stack
traces as `onOperatorDebug()` without its runtime cost, because the instrumentation happens
once in bytecode at class load rather than being intercepted on every subscribe. That is the
right tool for application-wide assembly traces in production.
...
known blocking points are covered without manual registration.

BlockHound detects blocking by **call type** — file I/O, `Thread.sleep`, certain locks — not
by the thread. A blocking JDBC call inside `Schedulers.boundedElastic()` is expected and
should not fire. If it does, an integration is missing from the classpath or an exemption is
misconfigured via `BlockHound.builder().allowBlockingCallsInside(...)`.
...
Reactor emits no native JFR events, so the bridge is a custom event committed inside the
global hook.
...
- Every hot source (Kafka, WebSocket, anything producing before a subscriber exists) has an
  explicit overflow strategy, never the implicit unbounded default.
