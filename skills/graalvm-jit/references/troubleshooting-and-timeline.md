...
| p99 fine, RSS higher than on OpenJDK by hundreds of MB, no heap growth                                         | libgraal isolate heap plus more JVMCI compiler threads (0.66 of `CICompilerCount` since 24)         | Compare RSS to `-Xmx` + Metaspace + code cache; NMT does not see the isolate (not verified here)           | Lower `-XX:JVMCINativeLibraryThreadFraction`, cap `CICompilerCount`, or raise the container limit — deliberately |
...
**Memory outside the heap.** libgraal is a native image with its own isolate heap and its
own GC (`gc=Serial GC` in the configuration line); it grows and shrinks with compilation
load and is not part of `-Xmx`, Metaspace or the code cache. Budget for it in the container
limit rather than discovering it as an OOM-kill under a start-up compilation burst. The
exact accounting in NMT is not verified here.
...
Two consequences for any 2026 decision follow directly. First, "Graal will be in OpenJDK
soon" is no longer a reason to defer or to accept; it is not coming. Second, the compiler
now changes monthly inside a fixed JDK base, so a result is a result for a GraalVM line —
record 25.0 versus 25.3 alongside the number, the way the JDK build is recorded for C2.
