...
| Benchmark shows no difference at all between "Graal" and "C2"                                                  | Possibly both sides ran C2: `-XX:-UseJVMCICompiler` in a shared flags file, or the wrong `JAVA_HOME` | JFR `jdk.Compilation` with `#threshold=0ms`: `compiler = "jvmci"` present on the Graal side?                         | Fix launch only if evidence shows the wrong compiler; equal scores may be a real result                         |
