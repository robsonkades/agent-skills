...
- **Binary**: pre-existing binaries that linked without error continue to link without error.
  This does not guarantee unchanged behaviour or successful execution of new call paths.
  Linkage errors can surface when a reference is resolved or a call executes
  (`NoSuchMethodError`, `AbstractMethodError`, `IncompatibleClassChangeError`,
  `IllegalAccessError`); classify the change using the JLS, not the exception class alone.
...
| --------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Add a method to a non-final class                                                       | OK                                                                                                          | risk: accidental override/static/return/throws clashes                                                      | risk                                                                          | Scan subclasses and inherited methods: additions can change dispatch, including existing super calls, without recompiling clients; recompilation can reveal source clashes                                               |
...
| Declare a constructor when the class had only an implicit no-argument constructor       | breaks old construction calls unless the no-argument constructor is retained                                | same risk for old construction calls and subclass constructors                                              | review initialization                                                         | An explicit constructor suppresses the implicit one; retain an explicit no-argument constructor with sufficient access and the supported initialization contract                                                         |
