...
additional current-class/superclass/superinterface and receiver constraints. Preserve the
original invocation semantics and verify any deliberate opcode transformation.
...
- Not discoverable by name — no `Class.forName`, absent from `getDeclaredClasses`, not
  resolvable through another class's constant pool by simple name. References can still reach
  it through the defining lookup, an instance's `getClass()` or other supported runtime APIs.
...
static Supplier<Integer> capturing(int x) { return () -> x; }
static Supplier<Integer> nonCapturing() { return () -> 42; }
```

Captured values appear in the invokedynamic factory descriptor and are retained for later
...
- **Records** (JEP 395, final in JDK 16) compile to a `final` class extending
  ...
  generated by this javac as `invokedynamic` sites bootstrapped by
  `java.lang.runtime.ObjectMethods` when not explicitly implemented. Records can supply their
  own method bodies; inspect those and the bootstrap before drawing conclusions about behavior.
  Bytecode alone still does not measure `toString` cost.
...
- **Pattern matching for `switch`** (JEP 441, final in JDK 21) over a sealed hierarchy uses
  ...
  that synthetic default can indicate binary evolution or an inconsistent runtime artifact set.
  Record-pattern accessor exceptions are another source of `MatchException`; inspect its cause
  and bytecode location before diagnosing the default branch.
...
javap -c -p MyClass                        # inspect whole method and backward branch targets for loop concat
...
`grep StringBuilder` also finds explicit builders and code compiled with `--release 8` or
`-XDstringConcat=inline` (verified: that flag brings the five `StringBuilder` calls back).
With javac 25's default concatenation strategy, look for a `makeConcatWithConstants`
