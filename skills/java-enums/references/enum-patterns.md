...
The abstract method makes a new constant a compile error until its body is written — the
property a `switch` cannot give you.
...
- Neither is a concurrent collection. For a shared mutable mapping keyed by an enum, use
  `ConcurrentHashMap` and accept the hash-based representation, or make the `EnumMap`
...
- `switch` over an enum compiles to a jump table over an internal ordinal-mapping array
  (`$SwitchMap`), which is why it is fast and also why it is compiled against the constants
