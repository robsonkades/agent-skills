...
| Primitive Obsession              | Replace Primitive with Object (record + compact constructor) | Replace Type Code (enum or sealed)                                                                     | Does the primitive carry a rule, a closed set, or demonstrated cross-concept confusion risk?                                         |
| Data Clumps                      | Introduce Parameter Object                                   | Extract Class if the group grows behaviour                                                             | Repeated co-travel plus a shared invariant or confusion risk; signature breakage alone proves nothing                                |
...
| Lazy Element                     | Inline Function / Inline Class                               | Collapse Hierarchy                                                                                     | A thin wrapper carrying a rule or demonstrated concept distinction can be a fix                                                      |
...
| Data Class                       | Move Function into it                                        | Encapsulate Collection; Replace Derived Variable with Query                                            | Misplaced behavior or bypassed invariants justify a finding; domain values, DTOs and projections may intentionally remain data       |
...
| Middle Man                       | Remove Middle Man                                            | Inline Function; Replace Superclass with Delegate                                                      | Forwarding is a lead; retain boundaries adding policy, translation, a narrower contract or substitution                              |
...
These are conditional preparation sequences, not universal prerequisites. Inspect live inputs,
outputs, mutation, exception timing and side effects before recommending reordering or splitting:

```text
Split Variable ─────────► Extract Method          (when one local represents distinct concepts)
Slide Statements ───────► Extract Method          (only when dependencies/effects permit movement)
...
Ordinary extracted method parameters need not be final or effectively final. The restriction
applies when capturing enclosing locals in lambdas or inner classes; extraction instead needs
an explicit way to return values that the caller still needs after the extracted block.
See [JLS 21 §15.27.2](https://docs.oracle.com/javase/specs/jls/se21/html/jls-15.html#jls-15.27.2).
