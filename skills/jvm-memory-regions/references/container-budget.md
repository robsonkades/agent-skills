...
heap size _and_ GC thread count. An isolated Windows Temurin 25.0.3 startup/exit probe with
`-Xms512m -Xmx512m -XX:+UseG1GC` reported about 79.1 MiB NMT GC committed with
`-XX:ActiveProcessorCount=24` (18 parallel GC workers), versus 46.5 MiB with a count of 2
(2 workers). These are controlled startup observations; attribute individual allocations and
production peaks separately. Under G1 on JDK 26
include two card tables ([JEP 522](https://openjdk.org/jeps/522), about 0.2% of heap each).
The second replaces another auxiliary structure, so measure the net GC-native footprint
change rather than assuming an unconditional additional 0.2% over the previous release.
...
# Check `jcmd <pid> help` for command availability on the deployed build.
...
| RSS grows while NMT stays flat                                 | Already-committed heap/native pages becoming resident, mapped-file working-set growth, or untracked native allocation | Compare heap used/committed and per-mapping residency before attributing an anonymous mapping to a native library | `System.map`/`System.dump_map` (Linux, Windows, macOS on 25); Linux `smaps` RSS/PSS; platform residency counters | Identify the growing resident domain first; unchanged NMT does not prove a native leak. For untracked allocation evidence, inspect `VM.dynlibs`/`jdk.NativeLibrary` and route to off-heap-memory or jni-and-ffm |
...
- [ ] Full budget summed by the arithmetic above: heap + Metaspace + code cache + stacks +
      ...
      fits the limit — not a percentage picked first. Account for the collector's native
      structures and release changes, including JDK 26 G1's replacement of an auxiliary
      structure with a second card table; measure the net effect as described above.
