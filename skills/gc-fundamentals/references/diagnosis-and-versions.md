...
| —   | Proposed: G1 becomes the default in every environment; Candidate, no target as of 2026-09-03             | JEP 523                       |
| —   | Draft: generational mode becomes Shenandoah's default; no JEP number or target as of 2026-09-03          | JDK-8379682                   |

Rows for 9–25 are established by the cited JEP or JBS issue and, where a flag is involved,
were executed on Temurin 25.0.3. The JDK 26 row records delivered JEP metadata but was not
executed here. The undated rows are proposals, not release promises; check their status
again before using them in an upgrade plan.
