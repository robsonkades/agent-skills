...
| ---------------------------- | --- | ------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| **Parameterize Constructor** | 379 | The collaborator the constructor `new`s becomes a constructor parameter                                             | A delegating old constructor can preserve callers; still check evaluation order, failures and ownership             |
| **Parameterize Method**      | 383 | Same move at method level: the object the method creates internally becomes an overload parameter                   | An extra caller contract; plan removal only if the overload is temporary                                            |
...
| Technique                               | p.  | What it does                                                                                                     | Cost / caveat                                                                                                              |
| --------------------------------------- | --- | ---------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| **Subclass and Override Method**        | 401 | Subclass the class under test _in the test_, overriding whatever is untestable                                   | Blocked by `final` classes/methods; a sealed hierarchy needs a permitted extension path                                    |
| **Extract and Override Call**           | 348 | Move an untestable call into its own method, then override that method in a test subclass                        | Adds an override contract; constrain test-only use and plan removal if temporary, or preserve a supported extension point  |
...
| **Link Substitution**       | 377 | Swap the implementation at link/classpath time                                                                | Distinguish hidden classpath shadowing from supported provider selection; verify the configuration and actual consumer path   |
