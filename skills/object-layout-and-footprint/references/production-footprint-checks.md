...
- JFR `jdk.ObjectCount` — per class, `count` and `totalSize`, taken at a GC. Its
  ...
  switched on, and it has the cost of a heap walk each time it fires.
...
`-Xmx32g` is therefore **off**, not the last value on: the margin below the 32 GB encoding
...
So the flag that saves 0–8 bytes per object **costs ~490 bytes per class**, and the 1 GB
default `CompressedClassSpaceSize` holds about **1.06 million** classes instead of about
**2 million** — the `Klass ID Range` line above is that limit stated by the JVM. Two
consequences:
...
| Objects grew after being hashed or locked                                                     | Not on 25 — measured unchanged (§5)                                                    | `Instrumentation.getObjectSize` before and after                                                                         | Look for a different cause: a lazily initialised field, a different class                                |
