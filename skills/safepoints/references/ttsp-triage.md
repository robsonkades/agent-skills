...
2. Each Java thread reacts at its next opportunity:
     compiled code (C1/C2)  → reads the polling page at the next emitted poll
...
| C1/C2, counted loop                 | one strip              | Strip mining puts the poll on the **outer** back-edge |
...
│       -XX:+SafepointTimeout -XX:SafepointTimeoutDelay=<low ms>  → stack of the late thread
│
├─ 4. Classify:
...
│       frequent non-GC operation (Deoptimize) → investigate the deoptimisation cause
│
