...
a cache, a report. It is derived data, and it must be treated as disposable — the moment a
projection holds something that cannot be recomputed from events, the event store has stopped
...
The check and the claim are one atomic statement. The obvious-looking alternative —
...
Assume every event will be delivered more than once. Subscriptions reconnect, consumers
restart mid-batch, and rebuilds re-read everything by design (`delivery-semantics`).

### Ordering

A projection that folds a running total requires per-stream order. Event stores provide it
within a stream; across streams there is a global order, but relying on it couples the
projection to the store's sequencing.
...
- The third row needs the append to return a position **in the coordinate system the projection
  ...
  or per partition. They are different coordinate systems, and the guarantee is per session, so
  it also needs sticky routing or a session-scoped pin (`consistency-models`).
...
Events are immutable and permanent; requirements are not. Every event type is a schema
commitment for the life of the system, and there is no option that makes an old event become
a new one.

### The changes, from cheapest to worst

**Add an optional field.** Old events deserialise with a default. This is the only cheap
change, and the reason to prefer a format that tolerates unknown and missing fields (JSON with
lenient deserialisation, Avro or Protobuf with defaults) over anything coupled to class
shape. Never Java serialisation: it welds the payload to the class it was written from
...
forward; keep handling the old one on read, forever. The write model gets a little uglier and
nothing breaks.

**Copy-and-replace the stream.** The last resort: read the old stream, write a new one with
transformed events, switch over, archive the original. It rewrites history — so it is
acceptable only for a genuine defect (events written by a bug), never for a requirement
change, and the original must be retained to keep the audit property honest.

### What to do about it in advance

- Version every event type from the first one written. Retrofitting a version field means
  guessing which of the unversioned events are which shape.
- Keep payloads flat and explicit. Deeply nested structures are far harder to upcast.
...
**Crypto-shredding** is the usual resolution: personal data in the payload is encrypted with a
key held per subject, outside the event store. Erasure deletes the key. The event remains,
ordered and countable, and its personal fields become permanently unreadable.
...
**What it does not do, and this is the part that gets systems into trouble: deleting the key
erases the copy in the event store and nothing else.** The live projection already persisted
...
Crypto-shredding is only an erasure mechanism if **no derived store holds the plaintext**.
