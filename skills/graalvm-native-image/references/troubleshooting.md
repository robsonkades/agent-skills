...
| ----------------------------------------------------- | ------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------- |
| Builder exits 137 without a Java exception            | SIGKILL, including container/host OOM kill or external termination                                      | cgroup OOM events, pod termination reason, host kernel log, build peak RSS versus limit                | for confirmed memory pressure, size builder memory/heap or reduce parallelism; otherwise repair the termination cause |
| `OutOfMemoryError` from builder                       | JVM heap, metaspace, native allocation, or thread/resource limit                                        | exact OOME detail, Java stack trace, build resource report and OS/container limits                     | adjust the exhausted resource; increase `-J-Xmx` only for demonstrated heap pressure, retaining native headroom       |
...
| Symptom                                            | Plausible causes                                                                                     | Distinguishing evidence                                                              | Remediation                                                                                            |
| -------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------ |
| `MissingReflectionRegistrationError`               | reflective type/member unregistered or its metadata condition inactive in exact mode                 | error names access and call path; check condition type and initialization order      | repair missing entry or activation contract; do not broaden member access to fix an inactive condition |
...
| `OutOfMemoryError` below container limit | heap/live-set pressure or a native allocation/thread/resource limit                                                    | exact OOME detail, heap/live set, native memory, thread count and OS limits                                  | fix or resize the exhausted resource; do not assume increasing heap fixes every OOME                         |
