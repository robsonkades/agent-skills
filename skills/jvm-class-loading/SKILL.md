...
description: >
  ...
  IllegalAccessError mentions "does not export", when a static initialiser does I/O, or when
  reducing cold start. Does not cover the Metaspace budget itself (jvm-memory-regions), JIT
  warm-up (jit-compilation), or heap leak analysis (jvm-gc-tuning). Metaspace internals are
  metaspace-internals and startup caching in depth is startup-cds-crac-leyden.
---
...
2. **Tell the two lookup failures apart.** `ClassNotFoundException` means not found;
   `NoClassDefFoundError` means found but its `<clinit>` already failed — go find the
   _original_ exception higher in the log.
3. **Check whether it is a module problem instead.** `IllegalAccessError` mentioning
   "does not export" is JPMS, not classpath; no JAR reorganisation fixes it.
...
- Keep `<clinit>` trivial. The first thread to touch the class pays the cost while holding
  ...
  ingredient of initialisation deadlock.
