...
| p99 up, no pause, no pacing                                             | Mutator evacuation: request threads copying cset objects they touch                                                      | CPU profile shows `ShenandoahRuntime::load_reference_barrier_strong` → `ShenandoahHeap::evacuate_object` in request threads | Check selected live bytes and CPU; reducing EvacReserve can worsen reclamation/headroom                                                        |
...
| Throughput lower than G1 by a margin on an allocation-heavy service     | Possible satb whole-live-set work, pacing, CPU contention or other costs                                                 | `gc+init` `Mode: Snapshot-At-The-Beginning`                                                                                 | `-XX:ShenandoahGCMode=generational`, then re-measure                                                                                           |
